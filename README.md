...
- **双阶段优化**: 论文润色 + 学术增强
- **智能分段**: 自动识别标题，跳过短段落
- **AIGC 风险筛查**: 改写完成后在会话详情页一键检测，辅助判断是否值得提交知网
- **使用限制**: 卡密系统，可配置使用次数
- **并发控制**: 队列管理，动态调整并发数
- **实时配置**: 修改配置无需重启服务
- **数据管理**: 可视化数据库管理界面
...
本工具内置了一个轻量级 AIGC 风险自查功能，定位为**辅助判断工具**，不作为最终裁定依据。

### 功能定位

- 改写完成后，在会话详情页切换到「AIGC 检测」标签即可使用
- 输出章节级风险分布和片段级证据，帮助快速定位需要人工修改的位置
- 支持关闭 LLM 增强模式，仅用本地文体特征进行零成本检测

### 使用限制（请务必了解）

- 本检测结果**不代表**知网、Turnitin 等商业系统的实际判定
- 风险分低不等于文本一定能通过外部检测
- 学术写作本身具有规范性，存在一定误报概率
- 建议作为「提交前参考」使用，而非替代专业检测系统

### 可选配置（`.env`）

```properties
# 检测专用模型（不填则复用 POLISH_* 配置）
DETECT_MODEL=
DETECT_API_KEY=
DETECT_BASE_URL=
```

> 详细数据策略和校准说明见 [`docs/detection/dataset_notes.md`](docs/detection/dataset_notes.md)
